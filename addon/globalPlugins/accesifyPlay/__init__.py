...
class SearchDialog(AccessifyDialog):
	...
	def __init__(self, parent, client):
		...
		self.resultsList.Bind(wx.EVT_CHAR_HOOK, self.on_results_char_hook)
	...
	def on_results_context_menu(self, evt):
		...
		if link:
			menu.Append(self.MENU_COPY_LINK.GetId(), _("Copy Link\tAlt+Shift+C"))
	...
	def on_results_char_hook(self, evt):
		...
		if key in (wx.WXK_RETURN, wx.WXK_NUMPAD_ENTER):
			...
			if (
				self.can_load_more
				and selection == len(self.results)
				and selection != wx.NOT_FOUND
			):
				self.onLoadMore()
				return
			self.onPlay(None)
	...
	def copy_selected_link(self, evt=None):
		index = self._ensure_active_selection(evt)
		if index is None:
			ui.message(_("No item selected."))
			return
	...
	def _select_item_from_event(self, evt):
		...
		hit = self.resultsList.HitTest(pos)
	...
	def _get_selected_index(
		self, activate_load_more=True, selection_override=None
	):
		...
		if selection == wx.NOT_FOUND:
			if (
				self._lastResultsSelection is not None
				and self._lastResultsSelection < len(self.results)
			):
	...
	def _get_selected_item(
		...
		index = self._get_selected_index(
			activate_load_more=activate_load_more,
	...
	def _ensure_active_selection(self, evt=None):
		manual_selection = None
		if evt is not None and hasattr(evt, "GetSelection"):
			manual_selection = evt.GetSelection()
		index = self._get_selected_index(
			activate_load_more=False, selection_override=manual_selection
		)
		if index is None or index < 0 or index >= len(self.results):
			return None
		current = self.resultsList.GetSelection()
		if current != index:
			try:
				self.resultsList.SetSelection(index)
			except Exception:
...
class PlayFromLinkDialog(AccessifyDialog):
	...
	def onPlay(self, evt):
		...
		if not uri:
			ui.message(_("Unable to play the provided link."))
			return
		ui.message(_("Playing..."))
		threading.Thread(
			target=self.client.play_item, args=(uri,)
		).start()
		self.Close()
...
class QueueListDialog(AccessifyDialog):
	def __init__(self, parent, client, queue_data):
		...
		self.queueList.Bind(wx.EVT_CHAR_HOOK, self._on_queue_char_hook)
	...
	def _on_queue_char_hook(self, evt):
		if evt.GetKeyCode() in (wx.WXK_RETURN, wx.WXK_NUMPAD_ENTER):
			self.play_selected_queue_item()
		else:
			evt.Skip()
...
class ManagementDialog(AccessifyDialog):
	...
	def init_saved_tracks_tab(self, parent_panel):
		...
		self._bind_list_activation(self.saved_tracks_list, self.play_selected_saved_track)
	...
	def init_followed_artists_tab(self, parent_panel):
		...
		self._bind_list_activation(self.followed_artists_list, self.play_selected_followed_artist)
	...
	def _on_saved_tracks_activate(self, evt):
		self.play_selected_saved_track()
	...
	def _on_followed_artists_activate(self, evt):
		self.play_selected_followed_artist()
	...
	def init_top_items_tab(self, parent_panel):
		...
		self._bind_list_activation(self.top_items_list, self.play_selected_top_item)
	...
	def _on_top_items_activate(self, evt):
		self.play_selected_top_item()
	...
	def init_saved_shows_tab(self, parent_panel):
		...
		self._bind_list_activation(self.saved_shows_list, self.play_selected_saved_show)
	...
	def _on_saved_shows_activate(self, evt):
		self.play_selected_saved_show()
	...
	def init_new_releases_tab(self, parent_panel):
		...
		self._bind_list_activation(self.new_releases_list, self.play_selected_new_release)
	...
	def _on_new_releases_activate(self, evt):
		self.play_selected_new_release()
	...
	def init_recently_played_tab(self, parent_panel):
		...
		self._bind_list_activation(self.recently_played_list, self.play_selected_recently_played)
	...
	def _init_shortcuts(self):
		...
		self._shortcutCloseId = wx.NewIdRef()
		...
		accel = wx.AcceleratorTable(
			[
				...
				(
					wx.ACCEL_ALT | wx.ACCEL_SHIFT,
					ord("C"),
					self._shortcutCopyId.GetId(),
				),
				(wx.ACCEL_ALT, ord("C"), self._shortcutCloseId.GetId()),
		...
		self.Bind(wx.EVT_MENU, self._handle_close_shortcut, id=self._shortcutCloseId.GetId())
		self.Bind(wx.EVT_MENU, self._handle_refresh_shortcut, id=self._shortcutRefreshId.GetId())
		self.Bind(
			wx.EVT_MENU,
			self._handle_new_playlist_shortcut,
			id=self._shortcutNewPlaylistId.GetId(),
		)
	...
	def _handle_close_shortcut(self, evt):
		self.Close()

	def _handle_refresh_shortcut(self, evt):
		handler = self._get_action_handler("refresh")
		if handler:
			handler()
		else:
			ui.message(_("Refresh is not available here."))

	def _handle_new_playlist_shortcut(self, evt):
	...
	def _open_create_playlist_dialog(self, evt=None):
		...
		dialog.Bind(wx.EVT_CLOSE, self._on_create_playlist_dialog_close)
		self._createPlaylistDialog = dialog
		dialog.Show()

	def _on_create_playlist_dialog_close(self, evt):
		self._createPlaylistDialog = None
		evt.Skip()
	...
	def on_playlist_details_updated(
		self, tree_item, name, description, public, collaborative
	):
		if not tree_item or not tree_item.IsOk():
			return
		data = self.playlist_tree.GetItemData(tree_item)
		if not isinstance(data, PlaylistTreeItemData):
			return
		data.name = name
		data.description = description
		data.public = public
		data.collaborative = collaborative
		self.playlist_tree.SetItemText(tree_item, name)
	...
	def _get_action_handler(self, action):
		...
		mapping = {
			self.playlist_tree: {
				...
				"refresh": self.load_playlists_to_tree,
			},
			self.saved_tracks_list: {
				...
				"refresh": self.load_saved_tracks,
			},
	...
	def _on_recently_played_activate(self, evt):
		self.play_selected_recently_played()
