...
class CreatePlaylistDialog(AccessifyDialog):
	def __init__(self, parent, client):
		...
		self.collabCheck = sHelper.addItem(wx.CheckBox(self, label=_("Collaborative (requires public off)")))
...
class AddToPlaylistDialog(AccessifyDialog):
	...
	def init_ui(self):
		...
		artists = ", ".join([a["name"] for a in self.current_track.get("artists", [])])
		...
		self.playlist_combobox = wx.ComboBox(panel, choices=playlist_names, style=wx.CB_READONLY)
	...
	def on_add_to_playlist(self, evt):
		...
		def _add():
			result = self.client.add_track_to_playlist(self.selected_playlist_id, self.current_track_uri)
			if isinstance(result, str):
...
class ArtistDiscographyDialog(AccessifyDialog):
	...
	def _load_more_all_tracks_thread(self):
		...
		has_more = bool(self._current_album_tracks) or (self._all_tracks_album_index < len(self.albums))
	...
	def _update_info_tab(self, info):
		...
		info_lines = [
			...
			_("Followers: {count}").format(count=followers_text or _("Unknown")),
			_("Popularity: {score}").format(score=popularity if popularity is not None else _("Unknown")),
			_("Genres: {genres}").format(genres=", ".join(genres) if genres else _("Unknown")),
	...
	def _on_add_to_playlist_selected(self, event, playlist_id, playlist_name):
		...
		ui.message(
			_("Adding '{track_name}' to '{playlist_name}'...").format(
	...
	def _on_add_album_to_playlist_action(self, playlist_id, playlist_name, album_id, album_name):
		ui.message(
			_("Adding tracks from '{album}' to '{playlist}'...").format(
...
class PlaylistTracksDialog(AccessifyDialog):
	...
	def _init_ui(self):
		...
		summary_lines = [_("Playlist: {name}").format(name=self.playlist.get("name", _("Unknown")))]
		if owner:
...
class ManagementDialog(AccessifyDialog):
	...
	def _handle_refresh(self, evt=None):
		...
		for tab_cfg in self.tabs_config.values():
			if tab_cfg["control"] == focused_control:
				tab_cfg.get("loader", lambda: None)()
				return
	...
	def init_generic_list_tab(
		self,
		key,
		title,
		loader_func,
		display_formatter,
		item_parser=lambda i: i,
	...
	def load_saved_tracks(self, initial_data=None):
		if initial_data is not None:
			self._populate_generic_list("saved_tracks", initial_data)
		else:
			threading.Thread(
				target=lambda: self._load_data_thread("saved_tracks", self.client.get_saved_tracks)
			).start()

	def load_saved_albums(self, initial_data=None):
		if initial_data is not None:
			self._populate_generic_list("saved_albums", initial_data)
		else:
			threading.Thread(
				target=lambda: self._load_data_thread("saved_albums", self.client.get_saved_albums)
			).start()

	def load_followed_artists(self, initial_data=None):
		if initial_data is not None:
			self._populate_generic_list("followed_artists", initial_data)
		else:
			threading.Thread(
				target=lambda: self._load_data_thread("followed_artists", self.client.get_followed_artists)
			).start()

	def load_saved_shows(self, initial_data=None):
		if initial_data is not None:
			self._populate_generic_list("saved_shows", initial_data)
		else:
			threading.Thread(
	...
	def _update_playlist_controls_state(self):
		...
		edit_button = [
			...
			if isinstance(c, wx.Button) and c.GetLabel() == _("&Edit Playlist Details")
		]
		if edit_button:
	...
	def _on_add_album_to_playlist_selected(self, event, playlist_id, playlist_name):
		...
		album_id = album.get("id")
		ui.message(
			_("Adding all tracks from '{album}' to '{playlist}'...").format(
		...
		if gui.messageBox(confirmation_msg, dialog_title, wx.YES_NO | wx.ICON_WARNING) == wx.YES:
			...
			def _delete():
				...
				if isinstance(result, str):
					...
					wx.CallAfter(
						ui.message,
						_("Playlist '{name}' deleted successfully.").format(name=playlist_data["name"]),
					)
	...
	def on_unfollow_playlist(self, evt):
		...
		playlist_data = self.user_playlists[selection_index]
		confirmation_msg = _("Are you sure you want to unfollow playlist '{name}'?").format(
		...
		if gui.messageBox(confirmation_msg, dialog_title, wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
			...
			def _unfollow():
				...
				if isinstance(result, str):
					...
					wx.CallAfter(
						ui.message, _("Unfollowed '{name}' successfully.").format(name=playlist_data["name"])
					)
					wx.CallAfter(self.load_playlists)
	...
	def _handle_reorder_track(self, direction):
		...
		target_track_label = self.playlist_tracks_list.GetString(
			to_index + 1 if direction == "up" else to_index - 1
		)
		if direction == "up":
	...
	def init_top_items_tab(self):
		...
		self.top_item_type_box = sHelper.addLabeledControl(
			_("Show:"), wx.ComboBox, choices=list(self.top_item_type_choices.keys()), style=wx.CB_READONLY
		)
	...
	def on_remove_from_library(self, evt):
		...
		if not item:
			return
		msg = _("Are you sure you want to remove '{track_name}' from your library?").format(
	...
	def on_remove_album_from_library(self, evt):
		...
		if not item:
			return
		msg = _("Are you sure you want to remove '{album_name}' from your library?").format(
	...
	def _remove_album_from_library_thread(self, album_id, album_name):
		...
		if isinstance(result, str):
			...
			wx.CallAfter(
				ui.message, _("Album '{album_name}' removed from your library.").format(album_name=album_name)
			)
	...
	def on_remove_show_from_library(self, evt):
		...
		if not item:
			return
		msg = _("Are you sure you want to remove '{show_name}' from your library?").format(
			show_name=item["name"]
		)
	...
	def _remove_show_from_library_thread(self, show_id, show_name):
		...
		if isinstance(result, str):
			...
			wx.CallAfter(
				ui.message, _("Show '{show_name}' removed from your library.").format(show_name=show_name)
			)
	...
	def on_view_discography(self, evt):
		...
		if target_id and target_name:
			dialog = ArtistDiscographyDialog(self, self.client, target_id, target_name, self.user_playlists)
			dialog.Show()
		else:
