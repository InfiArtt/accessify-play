...
- **The Library opens even when part of Spotify is unavailable**: the Library (`M`) loads nine lists before it opens, and if any one of them failed, it refused to open at all. New Releases depends on a part of Spotify's service that Spotify has announced it is retiring, so once that happens the whole Library would have stopped working. Now a list that fails shows why in its own tab and the rest of the Library opens as normal. It still stops with a single message if Spotify can't be reached at all, or you aren't logged in.
